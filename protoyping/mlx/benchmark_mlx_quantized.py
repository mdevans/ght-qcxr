...
print(f"🚀 Initializing HYBRID QUANTIZED Benchmark...")
...
print("✅ Weights Loaded (Float16).")

# --- 3. SMART QUANTIZATION (FIXED) ---
print("📦 Applying Hybrid 4-bit Compression...")

def smart_quantize(module):
    # Iterate over all attributes of the module
    for name, child in list(vars(module).items()):
        
        # 1. Handle Lists (like self.blocks)
        ...
        # 2. Handle Direct Modules
        if isinstance(child, nn.Module):
            # If it is a Linear layer, try to swap it
            if isinstance(child, nn.Linear):
                # Check compatibility: shape[-1] must be divisible by 64
                if child.weight.shape[-1] % 64 == 0:
                    q_layer = nn.QuantizedLinear.from_linear(child, bits=4, group_size=64)
                    setattr(module, name, q_layer)
            else:
                # Recurse deeper (e.g. into Attention or MLP blocks)
                smart_quantize(child)

smart_quantize(model)
print("✅ Hybrid Model Ready.")

# --- 4. BENCHMARK ---
print("🔥 Warming up (Compiling Hybrid Graph)...")
...
print("\n" + "="*40)
print(f"🍏 HYBRID PERFORMANCE REPORT")
print("="*40)

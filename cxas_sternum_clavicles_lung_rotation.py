import torch
import numpy as np
import cv2
import math
import re
from pathlib import Path
import torch.nn.functional as F
from PIL import Image

from cxas.models.UNet.backbone_unet import BackboneUNet

# --- CONFIGURATION ---
WEIGHTS_PATH = Path.home() / ".cxas" / "weights" / "UNet_ResNet50_default.pth"
IMAGE_DIR = Path("data/TB_Chest_Radiography_Database/Normal") 
OUTPUT_DIR_BASE = Path("segment/cxas_sternum_clavicles_lungs_masks")

DIR_PASS = OUTPUT_DIR_BASE / "pass"
DIR_REJECT = OUTPUT_DIR_BASE / "reject"
DIR_ERROR = OUTPUT_DIR_BASE / "error"

DEVICE = "mps" if torch.backends.mps.is_available() else "cpu"

# 🚀 The Correct Indices from your Dictionary!
CLAVICLE_LEFT_IDX = 31
CLAVICLE_RIGHT_IDX = 32
STERNUM_IDX = 29

LUNG_RIGHT_IDX = 135
LUNG_LEFT_IDX = 136
APICAL_RIGHT_IDX = 144
APICAL_LEFT_IDX = 148
UPPER_LOBE_LEFT_IDX = 150
UPPER_LOBE_RIGHT_IDX = 153

# --- HELPER FUNCTIONS ---

def normalize(array: torch.Tensor) -> torch.Tensor:
    array = (array - torch.tensor([0.485, 0.456, 0.406]).view(-1, 1, 1)) / (
        torch.tensor([0.229, 0.224, 0.225]).view(-1, 1, 1)
    )
    return array

def load_segmentation_model():
    print(f"🚀 Loading CXAS BackboneUNet on {DEVICE}...")
    model = BackboneUNet(model_name="UNet_ResNet50_default", classes=159)
    checkpoint = torch.load(WEIGHTS_PATH, map_location=DEVICE, weights_only=False)
    state_dict = checkpoint["model"]
    if list(state_dict.keys())[0].startswith("module."):
        state_dict = {k[len("module."):]: v for k, v in state_dict.items()}
    model.load_state_dict(state_dict, strict=False)
    model.to(DEVICE)
    model.eval()
    return model

def keep_largest_blob_only(binary_mask):
    mask_uint8 = binary_mask.astype(np.uint8)
    contours, _ = cv2.findContours(mask_uint8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return mask_uint8
    largest_contour = max(contours, key=cv2.contourArea)
    clean_mask = np.zeros_like(mask_uint8)
    cv2.drawContours(clean_mask, [largest_contour], -1, 1, thickness=cv2.FILLED)
    return clean_mask

def enforce_lung_boundary(clavicle_mask, safe_zone_mask, image_height):
    if np.max(safe_zone_mask) == 0 or np.max(clavicle_mask) == 0:
        return np.zeros_like(clavicle_mask)

    # Dilate heavily (8% of image height) to engulf the clavicles and shoulders
    kernel_size = int(image_height * 0.08)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    safe_zone_dilated = cv2.dilate(safe_zone_mask.astype(np.uint8), kernel, iterations=1)

    # Intersect using safe NumPy math
    cleaned_clavicle_bool = np.logical_and(clavicle_mask.astype(bool), safe_zone_dilated.astype(bool))
    cleaned_clavicle_uint8 = cleaned_clavicle_bool.astype(np.uint8)

    return keep_largest_blob_only(cleaned_clavicle_uint8)

def get_robust_medial_point(mask, side):
    kernel = np.ones((5,5), np.uint8) 
    eroded_mask = cv2.erode(mask, kernel, iterations=1)
    
    y_coords, x_coords = np.where(eroded_mask > 0)
    
    if len(x_coords) == 0:
        y_coords, x_coords = np.where(mask > 0)
        if len(x_coords) == 0:
            return (0, 0)

    idx = np.argmax(x_coords) if side == 'right' else np.argmin(x_coords)
    return (int(x_coords[idx]), int(y_coords[idx]))

# --- MATH & VISUALIZATION ---

def calculate_sternum_rotation(masks, image_width, image_height, filename):
    raw_left_clav = masks[CLAVICLE_LEFT_IDX]
    raw_right_clav = masks[CLAVICLE_RIGHT_IDX]

    metrics = {
        "valid": False, "status": "ERROR", "error_msg": "",
        "right_medial_pt": (0, 0), "left_medial_pt": (0, 0),
        "spine_line": (0.0, 1.0, 256.0, 256.0), 
        "dist_right": 0, "dist_left": 0, "pixel_diff": 0, "offset_ratio": 0.0
    }
    
    combined_lungs = (
        masks[LUNG_RIGHT_IDX] | masks[LUNG_LEFT_IDX] | 
        masks[APICAL_RIGHT_IDX] | masks[APICAL_LEFT_IDX] |
        masks[UPPER_LOBE_RIGHT_IDX] | masks[UPPER_LOBE_LEFT_IDX]
    )

    # Calculate the total pixel area of the lungs
    total_lung_area = np.sum(combined_lungs)
    
    # If there are no lungs, we can't do the area math
    if total_lung_area == 0:
        metrics["error_msg"] = "Empty Lung Masks"
        return metrics, masks[CLAVICLE_LEFT_IDX], masks[CLAVICLE_RIGHT_IDX], masks[STERNUM_IDX], combined_lungs
    
    y_coords, _ = np.where(combined_lungs > 0)
    if len(y_coords) > 0:
        top_y = np.min(y_coords)
        bottom_y = np.max(y_coords)
        cutoff_y = int(top_y + ((bottom_y - top_y) * 0.50))
        upper_safe_zone = np.zeros_like(combined_lungs, dtype=np.uint8)
        upper_safe_zone[:cutoff_y, :] = combined_lungs[:cutoff_y, :]
    else:
        upper_safe_zone = np.zeros_like(combined_lungs, dtype=np.uint8)

    left_clav_mask = enforce_lung_boundary(raw_left_clav, upper_safe_zone, image_height)
    right_clav_mask = enforce_lung_boundary(raw_right_clav, upper_safe_zone, image_height)
    sternum_mask = keep_largest_blob_only(masks[STERNUM_IDX])

    # 🚨 NEW: Clavicle Area Sanity Check
    # A true clavicle must be a substantial size relative to the lungs.
    # We enforce a minimum threshold (e.g., 0.5% of the total lung area).
    # If the mask is smaller than this, it's just a fragmented "splinter" or noise.
    MIN_CLAVICLE_RATIO = 0.01  # 0.1% of the lung area
    
    left_clav_area = np.sum(left_clav_mask)
    right_clav_area = np.sum(right_clav_mask)
    
    if (left_clav_area / total_lung_area) < MIN_CLAVICLE_RATIO:
        metrics["error_msg"] = f"Left Clavicle Too Small ({left_clav_area / total_lung_area:.2%})"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    if (right_clav_area / total_lung_area) < MIN_CLAVICLE_RATIO:
        metrics["error_msg"] = f"Right Clavicle Too Small ({right_clav_area / total_lung_area:.2%})"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    if np.max(left_clav_mask) == 0 or np.max(right_clav_mask) == 0:
        metrics["error_msg"] = "Missing Clavicle (Lung Boundary)"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    metrics["right_medial_pt"] = get_robust_medial_point(right_clav_mask, 'right')
    metrics["left_medial_pt"] = get_robust_medial_point(left_clav_mask, 'left')
    rx, ry = metrics["right_medial_pt"]
    lx, ly = metrics["left_medial_pt"]

    if rx == 0 or lx == 0:
        metrics["error_msg"] = "Failed to find medial points"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    if rx >= lx:
        metrics["error_msg"] = "Crossover Error (Right tip >= Left tip)"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    y_diff_ratio = abs(ry - ly) / image_height
    if y_diff_ratio > 0.15: 
        metrics["error_msg"] = "Vertical Disparity Error"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    sternum_y, sternum_x = np.where(sternum_mask > 0)
    if len(sternum_x) == 0:
        metrics["error_msg"] = "Empty Sternum Mask"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs
        
    points = np.column_stack((sternum_x, sternum_y)).astype(np.float32)
    line = cv2.fitLine(points, cv2.DIST_L2, 0, 0.01, 0.01)
    vx, vy, x0, y0 = line.flatten()
    vx, vy, x0, y0 = float(vx), float(vy), float(x0), float(y0)

    angle_from_vertical = math.degrees(math.atan2(abs(vx), abs(vy)))
    if angle_from_vertical < 5.0:
        vx = 0.0
        vy = 1.0
        M_sternum = cv2.moments(sternum_mask)
        if M_sternum["m00"] != 0:
            x0 = M_sternum["m10"] / M_sternum["m00"]

    metrics["spine_line"] = (vx, vy, x0, y0)

    dist_rx_to_line = abs((rx - x0) * vy - (ry - y0) * vx)
    dist_lx_to_line = abs((lx - x0) * vy - (ly - y0) * vx)
    
    if (dist_rx_to_line / image_width) > 0.15 or (dist_lx_to_line / image_width) > 0.15:
        metrics["error_msg"] = "Midline Proximity Error (>15%)"
        return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

    metrics["dist_right"] = dist_rx_to_line
    metrics["dist_left"] = dist_lx_to_line
    metrics["pixel_diff"] = abs(metrics["dist_left"] - metrics["dist_right"])
    metrics["offset_ratio"] = metrics["pixel_diff"] / image_width

    # Calculate the actual ratios to print to the terminal
    # left_ratio = left_clav_area / total_lung_area
    # right_ratio = right_clav_area / total_lung_area
    # print(f"  [INFO] {filename}: L Clavicle is {left_ratio:.2%}, R Clavicle is {right_ratio:.2%} of lung area.")
    
    metrics["valid"] = True
    metrics["status"] = "PASS" if metrics["offset_ratio"] <= 0.05 else "REJECT"

    return metrics, left_clav_mask, right_clav_mask, sternum_mask, combined_lungs

def visualize_results(img, masks_tuple, metrics, save_path):
    # Unpack the 4 masks now
    left_clav, right_clav, spine, lungs = masks_tuple
    
    overlay = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    mask_layer = overlay.copy()
    
    # Draw Lungs first (faint blue) so bones draw on top of them
    mask_layer[lungs > 0] = [255, 0, 0] 
    
    mask_layer[spine > 0] = [0, 165, 255] # Orange for Sternum
    mask_layer[left_clav > 0] = [255, 0, 255]
    mask_layer[right_clav > 0] = [0, 255, 255]

    alpha = 0.3
    cv2.addWeighted(mask_layer, alpha, overlay, 1 - alpha, 0, overlay)

    h, w = img.shape[:2]

    if metrics["valid"]:
        rx, ry = metrics["right_medial_pt"]
        lx, ly = metrics["left_medial_pt"]
        vx, vy, x0, y0 = metrics["spine_line"]
        
        if abs(vy) > 1e-6: 
            top_x = int(x0 - y0 * (vx / vy))
            bottom_x = int(x0 + (h - y0) * (vx / vy))
            cv2.line(overlay, (top_x, 0), (bottom_x, h), (0, 0, 255), 1, cv2.LINE_AA)

        def get_perpendicular_pt(px, py, vx, vy, x0, y0):
            t = (px - x0) * vx + (py - y0) * vy
            return int(x0 + t * vx), int(y0 + t * vy)

        r_intersect = get_perpendicular_pt(rx, ry, vx, vy, x0, y0)
        l_intersect = get_perpendicular_pt(lx, ly, vx, vy, x0, y0)

        cv2.line(overlay, (rx, ry), r_intersect, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.line(overlay, (lx, ly), l_intersect, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.circle(overlay, (rx, ry), 3, (0, 255, 0), -1, cv2.LINE_AA)
        cv2.circle(overlay, (lx, ly), 3, (0, 255, 0), -1, cv2.LINE_AA)
        
        text = f"Offset: {metrics['offset_ratio']:.1%} [{metrics['status']}]"
        color = (0, 255, 0) if metrics["status"] == "PASS" else (0, 0, 255)
        cv2.putText(overlay, text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1, cv2.LINE_AA)
    
    else:
        error_text = f"ERROR: {metrics.get('error_msg', 'Unknown Error')}"
        text_size = cv2.getTextSize(error_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
        text_x = (w - text_size[0]) // 2
        text_y = h - 30
        cv2.putText(overlay, error_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

    cv2.imwrite(save_path, overlay)

# --- BATCH PROCESSOR ---

def process_batch():
    DIR_PASS.mkdir(parents=True, exist_ok=True)
    DIR_REJECT.mkdir(parents=True, exist_ok=True)
    DIR_ERROR.mkdir(parents=True, exist_ok=True)
    
    model = load_segmentation_model()
    
    valid_extensions = {'.png', '.jpg', '.jpeg'}
    def natural_sort_key(path_obj):
        return [int(text) if text.isdigit() else text.lower() 
                for text in re.split(r'(\d+)', path_obj.name)]

    all_files = sorted([
        p for p in IMAGE_DIR.iterdir() 
        if p.is_file() and p.suffix.lower() in valid_extensions
    ], key=natural_sort_key)
    
    if not all_files:
        print(f"❌ No images found in {IMAGE_DIR}")
        return
    
    test_files = all_files[:1000]
    
    print(f"\n🧪 Testing Sternum-Based Rotation on {len(test_files)} Images...\n")
    print(f"{'FILENAME':<30} | {'LEFT DIST':<10} | {'RIGHT DIST':<10} | {'OFFSET %':<10} | {'RESULT'}")
    print("-" * 80)

    counts = {"pass": 0, "reject": 0, "error": 0}

    for image_path in test_files:
        filename = image_path.name
        img_str = str(image_path)
        
        try:
            pil_img = Image.open(image_path).convert(mode="RGB")
            orig_file_size = (pil_img.size[1], pil_img.size[0]) 
            orig_width = pil_img.size[0]
            orig_height = pil_img.size[1] 
        except Exception as e:
            print(f"❌ Skipping {filename} - Error loading: {e}")
            continue

        cv_img_raw = cv2.imread(img_str, cv2.IMREAD_GRAYSCALE)
        if cv_img_raw is None:
            continue

        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        cv_img_enhanced = clahe.apply(cv_img_raw)

        array = np.array(pil_img)
        array = np.transpose(array, [2, 0, 1])
        array = torch.tensor(array).float() / 255.0
        array = normalize(array).to(DEVICE)
        
        img_tensor = F.interpolate(array.unsqueeze(0), size=(512, 512))
        input_dict = {"data": img_tensor}

        with torch.no_grad():
            output_dict = model(input_dict)
            
        segmentation = output_dict["segmentation_preds"][0]
        pred = segmentation.float()
        pred = F.interpolate(pred.unsqueeze(0), size=orig_file_size, mode="nearest")
        masks = pred[0].bool().cpu().numpy()

        metrics, left_m, right_m, sternum_m, lungs_m = calculate_sternum_rotation(masks, orig_width, orig_height, filename)

        if not metrics["valid"]:
            status_str = "ANATOMY ERROR"
            out_dir = DIR_ERROR
            counts["error"] += 1
            print(f"{filename:<30} | {'---':<10} | {'---':<10} | {'---':<10} | 🚨 {status_str}")
        else:
            if metrics["status"] == "PASS":
                out_dir = DIR_PASS
                counts["pass"] += 1
                icon = "✅"
            else:
                out_dir = DIR_REJECT
                counts["reject"] += 1
                icon = "⚠️"
            
            print(f"{filename:<30} | {metrics['dist_left']:<10.1f} | {metrics['dist_right']:<10.1f} | {metrics['offset_ratio']:<10.1%} | {icon} {metrics['status']}")

        save_path = out_dir / f"qc_{filename}"
        visualize_results(cv_img_enhanced, (left_m, right_m, sternum_m, lungs_m), metrics, str(save_path))

    print("\n" + "="*40)
    print("🎯 BATCH PROCESSING COMPLETE")
    print(f"PASS: {counts['pass']} | REJECT: {counts['reject']} | ERROR: {counts['error']}")
    print("="*40)

if __name__ == "__main__":
    process_batch()